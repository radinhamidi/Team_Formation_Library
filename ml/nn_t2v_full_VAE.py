...
for fold_counter in range(1,k_fold+1):
    ...
    with open(result_output_name, 'a+') as file:
        ...
        for sample_x, sample_y in zip(x_test, y_sparse_test):
            ...
            end_time = time.time()
            sample_prediction = [[int(candidate[0]) for candidate in t2v_model_user.get_member_most_similar_by_vector(record[0], k_max)]]
    ...
    try:
        ...
        with open('../output/Models/{}_{}_Time{}_EncodingDim{}to{}_Fold{}_Loss{}_Epoch{}_kFold{}_BatchBP{}_BatchTraining{}.txt'
                        .format(dataset_name, method_name, time_str, embedding_dim_skill, embedding_dim_user, fold_counter,
                        'nan', epochs, k_fold, back_propagation_batch_size, training_batch_size), 'w') as f:
            with redirect_stdout(f):
                autoencoder.summary()
...
#     with open('../misc/{}_dim{}_{}_r_at_k_50.pkl'.format(method_name, embedding_dim_skill, embedding_dim_user), 'wb') as f:
#         pkl.dump(r_at_k_overall, f)
#     with open('../misc/{}_dim{}_{}_mapk_50.pkl'.format(method_name, embedding_dim_skill, embedding_dim_user), 'wb') as f:
#         pkl.dump(mapk, f)
#     with open('../misc/{}_dim{}_{}_ndcg_50.pkl'.format(method_name, embedding_dim_skill, embedding_dim_user), 'wb') as f:
#         pkl.dump(ndcg, f)
#     with open('../misc/{}_dim{}_{}_mrr_50.pkl'.format(method_name, embedding_dim_skill, embedding_dim_user), 'wb') as f:
#         pkl.dump(mrr, f)